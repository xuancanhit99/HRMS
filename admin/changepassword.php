...
if (strlen($_SESSION['aid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = mysqli_query($conn, "select ID from admin where ID='$adminid' and AdminPassword='$cpassword'");
        $row = mysqli_fetch_array($query);
        if ($row > 0) {
            $ret = mysqli_query($conn, "update admin set AdminPassword='$newpassword' where ID='$adminid'");
